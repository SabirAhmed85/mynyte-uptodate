...
app.factory('Profile', ['$http', 'Config', function($http, Config) {
    ...
    data.getListingsByBusinessType = function (_townId, _businessTypeId) {
		return $http(
            {
				method: 'GET', url:Config.CreateProfileUrl + '?action=getListings&_townId=' + _townId + '&_businessTypeId=' + _businessTypeId
			}
		);
	}
	...
	data.getWatchedListingsForProfile = function (_profileId) {
		return $http(
            {
				method: 'GET', url:Config.CreateProfileUrl + '?action=getWatchedListingsForProfile&_profileId=' + _profileId
			}
		);
	}
	...
	data.getLikedListingsForProfile = function (_profileId) {
		return $http(
            {
				method: 'GET', url:Config.CreateProfileUrl + '?action=getLikedListingsForProfile&_profileId='+_profileId
			}
		);
	}
...
app.factory('Messages',['$http', 'Config', function($http, Config) {
	...
	data.getMessageGroup = function (_groupId, _usersProfileId, currentMessageIndex) {
		return $http(
			{
				method: 'GET', url:Config.MessageUrl + '?action=getMessageGroup&_groupId=' + _groupId + '&_usersProfileId=' + _usersProfileId + '&currentMessageIndex=' + currentMessageIndex
			}
		);
	}
    ...
    data.addMessage = function (newMessage) {
		...
		return $http(
			{
				method: 'POST', url:Config.MessageUrl + '?action=addMessage&_groupId=' + newMessage._groupId + '&groupType='+ newMessage.groupType + '&_senderId=' + newMessage._senderId + '&messageText=' + newMessage.messageText + '&_relListingId=' + newMessage._relatedItemId + '&relListingType=' + newMessage.relatedItemType + profilesUrlString,
                headers: {'Content-Type': 'application/x-www-form-urlencoded'}
			}
		);
	}
    ...
    app.factory('Contacts', ['$http', 'Config', function($http, Config) {
        var data = {};
        data.getContacts = function () { console.log("wgf");
            return $http(
                {
        ...
        data.editContact = function (contactId, businessType, town, companyName, name, role, phone, email, website, note) {
            console.log('?action=edit&contactId=' + contactId + '&businessType=' + businessType + '&town=' + town + '&companyName=' + companyName + '&name=' + name + '&role=' + role + '&phone=' + phone + '&email=' + email + '&website=' + website + '&note=' + note);
            return $http(
                {
...
app.factory('Events', ['$http', 'Config', function($http, Config) {
	...
	data.getEventsByBusiness = function (_businessId, timeScale) {
		return $http(
            {
				method: 'GET', 
				url:Config.EventsUrl + '?action=getEvents&_businessId=' + _businessId + '&timeScale=' + timeScale,
                headers: {'Content-Type': 'application/x-www-form-urlencoded'}
			}
		);
	}
    data.getEventsByTown = function (_townId, _musicStyleId) {
		...
		return $http(
            {
				method: 'GET', url:Config.EventsUrl + '?action=getEvents&timeScale=future&_townId=' + stringAppend,
                headers: {'Content-Type': 'application/x-www-form-urlencoded'}
			}
		);
	}
    data.createEvent = function (_businessId, _businessPlaceId, eventTitle, description, eventDateTime, dressCode, guestListMax, dealsOnTheNight, extraInfo, eventHasGuestList, weekdayIndex, weeksAhead, _musicStyleIds) {
        console.log(_businessId, _businessPlaceId, eventTitle, description, eventDateTime, dressCode, guestListMax, dealsOnTheNight, extraInfo, eventHasGuestList, weekdayIndex, weeksAhead);
    ...
    data.updateEventEntryBookingByPerson = function (_entryBookingId, _eventDateId, addGuests, cancelled) {
        return $http(
            {
				method: 'POST', url:Config.TaxiUrl + '?action=updateEventEntryBookingByPerson&_entryBookingId=' + _entryBookingId + '&_eventDateId=' + _eventDateId + '&addGuests=' + addGuests + '&cancelled=' + cancelled
			}
		);
    }
  	return data;
...
app.factory('Offers', ['$http', 'Config', function($http, Config) {
	...
	data.getOffersByTownId = function (_townId, _profileId) {
		return $http(
            {
				method: 'GET', url:Config.OffersUrl + '?action=getOffers&_townId=' + _townId + '&_profileId=' + _profileId
			}
		);
	}
    ...
    data.createOffer = function (_businessId, _offerTypeId, _offerSubCategoryId, offerTitle, description, startDateTime, endDateTime, totalAvailable) {
		return $http(
            {
				method: 'GET', url:Config.OffersUrl + '?action=createOffer&_businessId=' + _businessId + '&_offerTypeId=' + _offerTypeId + '&_offerSubCategoryId=' + _offerSubCategoryId + '&offerTitle=' + offerTitle + '&description=' + description + '&startDateTime=' + startDateTime + '&endDateTime=' + endDateTime + '&totalAvailable=' + totalAvailable
			}
		);
	}
  	return data;
...
app.factory('MenuItems', ['$http', 'Config', function($http, Config) {
    ...
    data.createMenuItemTemplateOptionOption = function (_optionId, optionChoiceObject) {
        console.log(optionChoiceObject);
        return $http(
            {
				method: 'POST',
...
app.factory('Taxi', ['$http', 'Config', function($http, Config) {
    ...
    data.getTaxiBooking = function (_id, _businessId) {
		return $http(
            {
				method: 'GET', url:Config.TaxiUrl + '?action=getTaxiBooking&_id=' + _id + '&_businessId=' + _businessId
			}
		);
	}
    data.bookTaxi = function (_pickUpPlaceId, lookedUpPickUpLocation, pickUpAddressLine1, pickUpTown, pickUpPostCode, _dropOffPlaceId, lookedUpDropOffLocation, dropOffAddressLine1, dropOffTown, dropOffPostCode, totalPassengers, relContactName, relContactEmail, _relContactId) {
        return $http(
            {
				method: 'POST', url:Config.TaxiUrl + '?action=bookTaxi&pickUpPlaceId=' + _pickUpPlaceId + '&lookedUpPickUpLocation=' + lookedUpPickUpLocation + '&pickUpAddressLine1=' + pickUpAddressLine1 + '&pickUpTown=' + pickUpTown + '&pickUpPostCode=' + pickUpPostCode + '&dropOffPlaceId=' + _dropOffPlaceId + '&lookedUpDropOffLocation=' + lookedUpDropOffLocation + '&dropOffAddressLine1=' + dropOffAddressLine1 + '&dropOffTown=' + dropOffTown + '&dropOffPostCode=' + dropOffPostCode + '&totalPassengers=' + totalPassengers + '&relContactName=' + relContactName + '&relContactEmail=' + relContactEmail + '&_relContactId=' + _relContactId,
                headers: {'Content-Type': 'application/x-www-form-urlencoded'}
			}
		);
    }
...
app.factory('Movies', ['$http', 'Config', function($http, Config) {
    ...
    data.addMovie = function (name, description, firstShowingDate, lastShowingDate, _movieStyleIds, movieTrailerLink) {
		...
		return $http(
            {
				method: 'POST', url:Config.MovieUrl + '?action=addMovie' + movieStyleIdsString,
