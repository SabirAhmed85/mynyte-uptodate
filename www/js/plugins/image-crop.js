    ...
    .directive('imageCrop', function($rootScope) {
      ...
      return {
        ...
        link: function (scope, element, attributes) {
          ...
          var generalImageType = (scope.imageType == 'event_cover_photo' || scope.imageType == 'offer_cover_photo' || scope.imageType == 'cover_photo') ? 'cover_photo': 'profile_photo';
          ...
          scope.maxZoomedInLevel = 0;
          scope.maxZoomedOutLevel = 2;

          var $elm = element[0];

          var $canvas = $elm.getElementsByClassName('cropping-canvas')[0];
          var $handle = $elm.getElementsByClassName('zoom-handle')[0];
          var $handleInner = $elm.getElementsByClassName('zoom-handle-inner')[0];
          ...
          var zoom = scope.initialZoom ? Number(scope.initialZoom) : 1;
		  ...
		  function loadImage(base64ImageSrc) {
            ...
            handleSize(base64ImageSrc).then(function(base64ImageSrc) {
				...
				//otherwise, just return the image after pre-scaling it
				else {
                ...
                if (generalImageType == 'cover_photo' && img.width < 960) {
                    scope.imageUploadError = 'The photo you have selected is not wide enough. Please select a photo that is at least 960px in width and 640px in height.';
                    $rootScope.imageUploadError = 'err';
                    scope.updateButtonDisabled = true;
                }
                else if (generalImageType == 'cover_photo' && img.height < 640) {
                    scope.imageUploadError = 'The photo you have selected is too short in height. Please select a photo that is at least 960px in width and 640px in height.';
                    $rootScope.imageUploadError = 'err';
                    scope.updateButtonDisabled = true;
                }
                else {
                    scope.imageUploadError = null;
                    $rootScope.imageUploadError = null;
          ...
          $img.onload = function() {
			...
			if(imgWidth >= imgHeight) {
				scope.maxZoomedInLevel = ($canvas.height - padding) / imgHeight;
			} else {
				scope.maxZoomedInLevel = ($canvas.width - padding) / imgWidth;
			}
			...
			moveImage(initialX, initialY, 'resetCoords');
            zoomImage((($rootScope.dataScale * $img.width)/$img.width) - 1, "initial");
          ...
          zoomImage = function (val, state) {
            ...
            if (proposedZoomLevel > scope.maxZoomedOutLevel) {
              // image wont fill whole canvas!
              proposedZoomLevel = scope.maxZoomedOutLevel;
            }
            if (proposedZoomLevel < scope.maxZoomedInLevel) {
              // image is too far zoomed in, it's gonna get pretty pixelated!
              proposedZoomLevel = scope.maxZoomedInLevel;
            }
            
            zoom = proposedZoomLevel;
            scope.zoomProportion = (zoom - scope.maxZoomedInLevel)/(scope.maxZoomedOutLevel - scope.maxZoomedInLevel);
            scope.zoom = zoom;
            ...
            $($handleInner).css({'left': 'calc(' +scope.zoomProportion + ' * 97%)'});
            /* Bring this back in after finding out about fileReader Cancel function */
            //$rootScope.appLoading = state == 'initial' && $rootScope.appLoading == true ? false: $rootScope.appLoading;
          }
          ...
          function calcZoomLevel(diffX, diffY) {
            ...
            var newZoomDiff = to2Dp((scope.maxZoomedOutLevel - scope.maxZoomedInLevel) * zoomGestureRatio * zoomWeight);
          ...
          scope.doCrop = function() {
            ...
            $rootScope.$broadcast('cover-photo-uploaded', {imageX: imageX, imageY: imageY, imageZoom: zoom, currentViewName: $rootScope.currentViewName});
            alert('imageX: ' +imageX + ', imageY: ' +imageY +', imageZoom: ' + zoom);
            scope.step = 3;
          };
      ...
      $rootScope.$on('reset-image-crop', function (event, data) {
        scope.crop = false;
        scope.step = 1;
      });
    });
